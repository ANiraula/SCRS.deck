...
sensitivity <- Data %>% select(npl_pct_up, npl,npl_pct_down) %>% mutate_all(as.numeric)
...
reDiscount <- function(data, sensitivity){
  ...
  sens <- sens %>% mutate(aal.duration = ((npl_pct_down/npl_pct_up)^(0.5)-1)*100,
                          aal.convexity = ((npl_pct_down*npl_pct_up)/(npl^2)-1)*100,
...
Data <- data.frame(
                   ...
                   amo.period = c(17,21,25, 26,30,30,29,29,30,30,29, 30,30,29.7,30.4,36,30,29,28,27,26)
...
########areaPlot
  ...
  amo.period <- ggplot2::ggplot(data, ggplot2::aes(x = year)) + 
    ...
    ggplot2::scale_y_continuous(breaks = seq(0, 40, by = 4),limits = c(0, 40.2), expand = c(0, 0))+
...
-   Only **`r vest.prob*100`%** of new SCRS employees will stay for 8 years to vest.

    </p>
    
-   Only **`r vest.20*100`%** of the same employees will be on the job after 20 years.
    
    </p>

-   And only **`r retire.prob*100`%** of the same employees will be on the job after 36 years needed to qualify for full retirement (YOS + AGE = 90)
