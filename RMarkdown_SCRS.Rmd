...
data <- as.data.table(data)
  ...
  ggplot2::ggplot(graph, ggplot2::aes(x = year, y = yaxisScale * 
    ...
    y = labelY)+
    theme(legend.direction = "vertical", legend.box = "horizontal", 
    legend.position = c(1.04, 0.14)) + labs(title = paste(title), caption = ifelse(isTRUE(caption), 
    paste("reason.org/pensions"), paste(""))) + ggplot2::theme(axis.ticks = if (isFALSE(ticks)) {
    ggplot2::element_blank()
  }
  else {
...
But this value spikes to **$`r pw.32`K** or **`r pw.32.p`%** jump just after 4 more years (long enough to receive Full Benefits)
